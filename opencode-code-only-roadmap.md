# OpenCode: Code-Only Feature Roadmap
...
The goal is to make OpenCode substantially more capable than a traditional single-agent coding CLI by improving orchestration, reliability, context management, verification, developer experience, and local automation.
...
An explicit graph lets OpenCode:
...
This gives OpenCode a project-specific way to evaluate changes to:
...
Run an OpenCode process locally that can monitor approved tasks.
...
Hooks let developers customize behavior without modifying OpenCode core.
...
> **OpenCode should not merely generate code. It should execute, verify, explain, and recover from software-engineering tasks.**

The biggest opportunity is to turn OpenCode into an **open, model-agnostic runtime for software-engineering agents**.
...
This direction can be implemented incrementally and largely with code, while preserving OpenCode's existing model/provider flexibility.
