import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@noxijent-ai/client/promise"
import { Button } from "@noxijent-ai/ui/button"
import { useDialog } from "@noxijent-ai/ui/context/dialog"
import { Dialog } from "@noxijent-ai/ui/dialog"
import { Icon } from "@noxijent-ai/ui/icon"
import { IconButton } from "@noxijent-ai/ui/icon-button"
import { List, type ListRef } from "@noxijent-ai/ui/list"
import { ProviderIcon } from "@noxijent-ai/ui/provider-icon"
import { Spinner } from "@noxijent-ai/ui/spinner"
import { Tag } from "@noxijent-ai/ui/tag"
import { TextField } from "@noxijent-ai/ui/text-field"
import { ButtonV2 } from "@noxijent-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@noxijent-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@noxijent-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "noxijent-go") return language.t("dialog.provider.noxijentGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "noxijent"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.noxijent.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["noxijent", "noxijent-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "noxijent" || provider.id === "noxijent-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "noxijent"
                                ? "dialog.provider.noxijent.tagline"
                                : "dialog.provider.noxijentGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "noxijent" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "noxijent-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "noxijent"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.noxijentZen.line1")}</div>
              <div>{language.t("provider.connect.noxijentZen.line2")}</div>
              <div>
                {language.t("provider.connect.noxijentZen.visit.prefix")}
                <ExternalLink
                  href="https://noxijent.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.noxijentZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.noxijentZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "noxijent"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.noxijentZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.noxijentZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.noxijentZen.visit.prefix")}
                <ExternalLink href="https://noxijent.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.noxijentZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.noxijentZen.visit.suffix")}
              </div>
