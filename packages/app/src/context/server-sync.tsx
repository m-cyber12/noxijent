...
import { NormalizedProviderListResponse } from "@noxijent-ai/session-ui/context"
...
import type {
  ...
  McpServer,
  SessionActiveOutput,
} from "@noxijent-ai/client/promise"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: NoxijentClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => NoxijentClient,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => NoxijentClient,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, NoxijentClient>()
