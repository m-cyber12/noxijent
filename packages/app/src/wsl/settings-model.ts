...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-noxijent" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslNoxijentAction(check?: WslNoxijentCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installNoxijent"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateNoxijent"
}
...
export function addServerViewModel(input: {
  ...
  const noxijentCheck = selectedDistro ? (state?.noxijentChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.noxijentChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateNoxijent" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.noxijentMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installNoxijent" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  noxijentCheck: WslNoxijentCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingNoxijent =
    input.state?.job?.kind === "install-noxijent" && input.state.job.distro === input.selectedDistro
  if (!ready || probingNoxijent) {
    return {
      variant: "contrast",
  ...
  if (!addServerNoxijentReady(input.noxijentCheck)) {
    const update = !!input.noxijentCheck?.resolvedPath && input.noxijentCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingNoxijent
        ? { key: "wsl.onboarding.updatingNoxijent" }
        : update
          ? { key: "wsl.onboarding.updateNoxijent" }
          : { key: "wsl.onboarding.installNoxijent" },
      disabled: !!input.state?.job || input.adding,
      action: "install-noxijent",
      loading: installingNoxijent,
...
function addServerNoxijentReady(check: WslNoxijentCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.noxijentChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.noxijentChecks[item.name]) return [`noxijent:${item.name}`]
    return []
  })
