...
import { wslNoxijentAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.noxijentChecks[item.config.distro]
          const noxijentAction = () => wslNoxijentAction(check())
          const busy = () => wsl.data?.job?.kind === "install-noxijent" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={noxijentAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installNoxijent(item.config.distro))}
                    >
