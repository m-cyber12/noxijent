...
import { createNoxijentClient } from "@noxijent-ai/sdk/v2/client"
import { ServerAuth } from "@noxijent-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createNoxijentClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@noxijent/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createNoxijentClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createNoxijentClient({ baseUrl: connection.url, headers: connection.headers })
    })
