...
This is the living design and status document for `@noxijent-ai/codemode` and its existing V2 Noxijent adapter.
...
`packages/noxijent/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
