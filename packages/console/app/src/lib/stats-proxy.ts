...
import { Resource, waitUntil } from "@noxijent-ai/console-resource"
import { LOCALE_HEADER, cookie, localeFromRequest, route, tag } from "~/lib/language"

const dataPath = "/data"
const statsCacheParam = "__noxijent_stats_locale"
...
export async function statsProxy(evt: APIEvent) {
  ...
  targetUrl.hostname = Resource.App.stage === "production" ? "stats.noxijent.ai" : "stats.dev.noxijent.ai"
  targetUrl.port = ""
...
function withStatsCacheStatus(response: Response, status: "HIT" | "MISS") {
  const headers = new Headers(response.headers)
  headers.set("x-noxijent-stats-cache", status)
