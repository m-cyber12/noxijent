...
import { and, Database, eq, isNull, lt, or, sql } from "@noxijent-ai/console-core/drizzle/index.js"
import { KeyTable } from "@noxijent-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@noxijent-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@noxijent-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@noxijent-ai/console-core/util/date.js"
import { Identifier } from "@noxijent-ai/console-core/identifier.js"
import { Billing } from "@noxijent-ai/console-core/billing.js"
import { Actor } from "@noxijent-ai/console-core/actor.js"
import { WorkspaceTable } from "@noxijent-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@noxijent-ai/console-core/model.js"
import { Subscription } from "@noxijent-ai/console-core/subscription.js"
import { BlackData } from "@noxijent-ai/console-core/black.js"
import { UserTable } from "@noxijent-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@noxijent-ai/console-core/schema/model.sql.js"
...
import { LiteData } from "@noxijent-ai/console-core/lite.js"
...
import { Workspace } from "@noxijent-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-noxijent-session") ?? ""
    const requestId = input.request.headers.get("x-noxijent-request") ?? ""
    const ocClient = input.request.headers.get("x-noxijent-client") ?? ""
    const projectId = input.request.headers.get("x-noxijent-project") ?? ""
    ...
    if (authInfo && opts.modelList === "lite" && requiresGoTrainingConsent(modelInfo.id) && !authInfo.allowTraining)
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://noxijent.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const providerRequest = async () => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          if (!isNewInference) {
            headers.delete("x-noxijent-session")
            headers.delete("x-noxijent-project")
            headers.delete("x-noxijent-client")
            headers.delete("x-noxijent-request")
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-noxijent-endpoint-id")
        const resEndpointModelId = res.headers.get("x-noxijent-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      ...
      try {
        const consoleGoUrl = `https://noxijent.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://noxijent.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://noxijent.ai/workspace/${authInfo.workspaceID}/members`
