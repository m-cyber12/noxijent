...
import type { IntegrationOAuthMethodRegistration } from "@noxijent-ai/plugin/v2/effect/integration"
import { define } from "@noxijent-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@noxijent-ai/sdk/v2/types"
...
const defaultServer = "https://noxijent.ai/console"
const clientID = "noxijent-cli"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    integrationID: Integration.ID.make("noxijent"),
    method: {
      id: methodID,
      type: "oauth",
      label: "Noxijent Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const NoxijentPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "noxijent",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("NoxijentPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("noxijent")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Noxijent provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("noxijent", (integration) => {
        integration.name = "Noxijent"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "noxijent", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("noxijent")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("noxijent")
      ...
      const item = catalog.provider.get(ProviderV2.ID.noxijent)
      if (!item) return
      const hasKey = Boolean(process.env.NOXIJENT_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("noxijent")),
