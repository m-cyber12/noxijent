...
import { ToolFailure } from "@noxijent-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@noxijent/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.NOXIJENT_WEBSEARCH_PROVIDER === "exa" || process.env.NOXIJENT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.NOXIJENT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("NOXIJENT_EXPERIMENTAL") || truthy("NOXIJENT_ENABLE_EXA") || truthy("NOXIJENT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("NOXIJENT_ENABLE_PARALLEL") || truthy("NOXIJENT_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `noxijent/${InstallationVersion}`,
