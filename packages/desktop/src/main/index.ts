...
const TEST_ONBOARDING = process.env.NOXIJENT_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.NOXIJENT_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.NOXIJENT_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.noxijent.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `noxijent-onboarding-${randomUUID()}`)
    rmSync(root, { recursive: true, force: true })
    ;["data", "config", "cache", "state", "desktop", "session"].forEach((dir) =>
      mkdirSync(join(root, dir), { recursive: true }),
    )
    process.env.NOXIJENT_DB = ":memory:"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Noxijent Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("noxijent://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("noxijent")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.NOXIJENT_PORT
      if (fromEnv) {
