...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.noxijent.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `noxijent.settings.dat` is special: it maps to the `noxijent.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"noxijent.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // noxijent.settings.dat → the electron settings store ("noxijent.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "noxijent.global.dat").
  const storeName = filename === "noxijent.settings.dat" ? "noxijent.settings" : filename
