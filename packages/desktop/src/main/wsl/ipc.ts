...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-noxijent", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installNoxijent(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-noxijent", unavailable)
