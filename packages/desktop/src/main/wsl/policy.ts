import type { WslDistroProbe, WslNoxijentCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  noxijentChecks: Record<string, WslNoxijentCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextNoxijentChecks = { ...noxijentChecks }
  delete nextDistroProbes[distro]
  delete nextNoxijentChecks[distro]
  return { distroProbes: nextDistroProbes, noxijentChecks: nextNoxijentChecks }
}
