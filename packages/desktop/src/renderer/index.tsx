...
import {
  ...
  useWslServers,
  useLanguage,
} from "@noxijent-ai/app"
import type { UpdaterState } from "@noxijent-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@noxijent-ai/ui/logo"
import { useTheme } from "@noxijent-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.NOXIJENT_CHANNEL === "prod" &&
...
const deepLinkEvent = "noxijent:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NOXIJENT__ ??= {}
  const pending = window.__NOXIJENT__.deepLinks ?? []
  window.__NOXIJENT__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `noxijent.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://noxijent.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("noxijent.global.dat").getItem("language")
