import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@noxijent-ai/sdk/v2"
import { SessionTurn } from "@noxijent-ai/session-ui/session-turn"
import { SessionReview } from "@noxijent-ai/session-ui/session-review"
import { DataProvider } from "@noxijent-ai/session-ui/context"
import { FileComponentProvider } from "@noxijent-ai/ui/context/file"
import { WorkerPoolProvider } from "@noxijent-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@noxijent-ai/ui/logo"
import { IconButton } from "@noxijent-ai/ui/icon-button"
import { ProviderIcon } from "@noxijent-ai/ui/provider-icon"
import { iife } from "@noxijent-ai/core/util/iife"
import { Binary } from "@noxijent-ai/core/util/binary"
import { NamedError } from "@noxijent-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@noxijent-ai/ui/tabs"
import { MessageNav } from "@noxijent-ai/session-ui/message-nav"
import { FileSSR } from "@noxijent-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@noxijent-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/noxijent-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Noxijent</Title>
              </Show>
              <Meta name="description" content="noxijent - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/noxijent"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://noxijent.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
