...
export const verifyPackage = async (archive: string) => {
  ...
  try {
    ...
    await writeFile(
      ...
      `import { HttpRecorder } from "@noxijent-ai/http-recorder"
...
`,
    ...
    await run(
      [
        ...
        "--input-type=module",
        "-e",
        'import("@noxijent-ai/http-recorder").then((module) => { const root = Object.keys(module).sort(); const namespace = Object.keys(module.HttpRecorder).sort(); if (JSON.stringify(root) !== JSON.stringify(["HttpRecorder"])) throw new Error(`Unexpected root exports: ${root}`); if (JSON.stringify(namespace) !== JSON.stringify(["http", "socket"])) throw new Error(`Unexpected namespace exports: ${namespace}`) })',
      ],
      directory,
    )
