...
Practical reference for a future `packages/server` split after the noxijent
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/noxijent`.
...
Until enough shared service code lives outside `packages/noxijent`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/noxijent`

It should not import `packages/noxijent` services while `packages/noxijent`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on noxijent-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/noxijent` implementation details.
