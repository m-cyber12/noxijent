...
Trim `packages/noxijent/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/noxijent/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/noxijent`.
- `[x]` `bun typecheck` from `packages/noxijent`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/noxijent/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/noxijent`.
- `[x]` `bun dev generate > /tmp/noxijent-openapi.json` from `packages/noxijent`.
...
- Edit `packages/noxijent/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/noxijent-openapi.json` from `packages/noxijent`.
...
- Check source schemas in files like `packages/noxijent/src/session/schema.ts`, `packages/noxijent/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/noxijent/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/noxijent/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
