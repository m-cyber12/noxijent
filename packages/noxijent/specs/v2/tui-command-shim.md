...
Delete `packages/noxijent/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/noxijent/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/noxijent/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/noxijent`
- TUI plugin loader tests in `packages/noxijent` if runtime plugin API wiring changed
