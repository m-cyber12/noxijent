...
import type { AssistantMessage, Message, NoxijentClient, Session, SessionMessageResponse } from "@noxijent-ai/sdk/v2"
...
export const AuthMethodID = "noxijent-login"
...
export class Service extends Context.Service<Service, Interface>()("@noxijent/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `noxijent auth login` in the terminal",
      name: "Login with noxijent",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: NoxijentClient) {
...
function makeUsageService(sdk: NoxijentClient) {
...
async function loadDirectorySnapshot(sdk: NoxijentClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, noxijent provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const noxijentProvider = providers[ProviderV2.ID.make("noxijent")]
  const noxijentModel = noxijentProvider ? Provider.sort(Object.values(noxijentProvider.models))[0] : undefined
  if (noxijentProvider && noxijentModel) return { providerID: noxijentProvider.id, modelID: noxijentModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Noxijent service failure", service })
}
