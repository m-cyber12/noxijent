import type { PermissionV1 } from "@noxijent-ai/core/v1/permission"
import { FSUtil } from "@noxijent-ai/core/fs-util"
// CLI entry point for `noxijent run` and `noxijent --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`noxijent --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`noxijent --mini --attach`): connects to a running
//      noxijent server and runs interactive mode against it.
//
...
import { createNoxijentClient, type NoxijentClient, type ToolPart } from "@noxijent-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<NoxijentClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run noxijent with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running noxijent server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NOXIJENT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to NOXIJENT_SERVER_USERNAME or 'noxijent')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: NoxijentClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: NoxijentClient, sessionID: string) {
      ...
      async function current(sdk: NoxijentClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: NoxijentClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: NoxijentClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: NoxijentClient) {
        ...
        async function loop(client: NoxijentClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createNoxijentClient({
        baseUrl: "http://noxijent.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
