// Shared type vocabulary for the direct interactive mode (`noxijent --mini`).
//
...
import type { NoxijentClient, PermissionRequest, QuestionRequest, ToolPart } from "@noxijent-ai/sdk/v2"
import type { TuiConfig } from "@noxijent-ai/tui/config"
...
type PromptModel = Parameters<NoxijentClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<NoxijentClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<NoxijentClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<NoxijentClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<NoxijentClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<NoxijentClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<NoxijentClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<NoxijentClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<NoxijentClient["question"]["reject"]>[0]
