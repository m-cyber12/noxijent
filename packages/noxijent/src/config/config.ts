...
import { httpClient } from "@noxijent-ai/core/effect/app-node-platform"
import { serviceUse } from "@noxijent-ai/core/effect/service-use"
...
import { Flag } from "@noxijent-ai/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { InstallationLocal, InstallationVersion } from "@noxijent-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@noxijent-ai/core/v1/config/console-state"
import { FSUtil } from "@noxijent-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@noxijent-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@noxijent-ai/core/v1/config/config"
import { RemoteAuthError } from "@noxijent-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@noxijent-ai/core/v1/config/permission"
import { ConfigPluginV1 } from "@noxijent-ai/core/v1/config/plugin"
...
import { Npm } from "@noxijent-ai/core/npm"
...
export class Service extends Context.Service<Service, Interface>()("@noxijent/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["noxijent.jsonc", "noxijent.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://noxijent.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.NOXIJENT_CONFIG && !Flag.NOXIJENT_CONFIG_DIR && !Flag.NOXIJENT_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://noxijent.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "noxijent.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "noxijent.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://noxijent.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "NOXIJENT_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/noxijent`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://noxijent.ai/config.json"
        ...
        if (Flag.NOXIJENT_CONFIG) {
          yield* merge(Flag.NOXIJENT_CONFIG, yield* loadFile(Flag.NOXIJENT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.NOXIJENT_CONFIG })
        }

        if (!Flag.NOXIJENT_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("noxijent", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.NOXIJENT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from NOXIJENT_CONFIG_DIR", { path: Flag.NOXIJENT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".noxijent") || dir === Flag.NOXIJENT_CONFIG_DIR) {
            for (const file of ["noxijent.json", "noxijent.jsonc"]) {
          ...
          // Auto-discovered plugins under `.noxijent/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.NOXIJENT_CONFIG_CONTENT) {
          const source = "NOXIJENT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.NOXIJENT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from NOXIJENT_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["NOXIJENT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("NOXIJENT_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.NOXIJENT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.NOXIJENT_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("NOXIJENT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.NOXIJENT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.NOXIJENT_DISABLE_PRUNE) {
