...
const TUI_SCHEMA_URL = "https://noxijent.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from noxijent.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const noxijent = await noxijentFiles(input)
  for (const file of noxijent) {
...
async function noxijentFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "noxijent"),
    ...(await Filesystem.findUp(["noxijent.json", "noxijent.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "noxijent"))
  }
  if (Flag.NOXIJENT_CONFIG) files.push(Flag.NOXIJENT_CONFIG)
