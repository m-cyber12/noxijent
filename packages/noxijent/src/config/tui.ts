...
import { isRecord } from "@noxijent-ai/tui/util/record"
...
import { TuiKeybind } from "@noxijent-ai/tui/config/keybind"
...
import { TuiConfig } from "@noxijent-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@noxijent/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old noxijent.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.noxijent`
  // folders between cwd and home, and NOXIJENT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NOXIJENT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NOXIJENT_TUI_CONFIG override, if set.
  if (Flag.NOXIJENT_TUI_CONFIG) {
    const configFile = Flag.NOXIJENT_TUI_CONFIG
  ...
  // 4. `.noxijent` directories (and NOXIJENT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".noxijent") || dir === Flag.NOXIJENT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".noxijent") && dir !== Flag.NOXIJENT_CONFIG_DIR) continue
