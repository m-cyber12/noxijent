...
import { NonNegativeInt, PositiveInt } from "@noxijent-ai/core/schema"
import { ConfigAttachmentV1 } from "@noxijent-ai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@noxijent-ai/core/v1/config/lsp"
import { InvalidError } from "@noxijent-ai/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by Noxijent V1. Use V1 "permission" rules or run noxijent2.',
      })),
    })
