...
export const DEPRECATED_PLUGIN_PACKAGES = ["noxijent-openai-codex-auth", "noxijent-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, noxijentVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(noxijentVersion) || semver.major(noxijentVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.noxijent
  if (typeof range !== "string") return
  if (!semver.satisfies(noxijentVersion, range)) {
    throw new Error(`Plugin requires noxijent ${range} but running ${noxijentVersion}`)
  }
}
