...
import { errorData, errorMessage } from "@noxijent-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@noxijent-ai/tui/context/theme"
import { Global } from "@noxijent-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@noxijent-ai/core/util/flock"
import { Flag } from "@noxijent-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@noxijent-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@noxijent-ai/core/v1/config/plugin"
import { createCommandShim } from "@noxijent-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@noxijent-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".noxijent"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".noxijent", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".noxijent", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NOXIJENT_PURE ? [] : pluginOrigins
    if (Flag.NOXIJENT_PURE && pluginOrigins.length) {
    }
