...
import { NoxijentHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for noxijent serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NoxijentHttpApi)
...
export const PublicApi = NoxijentHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "noxijent",
    version: "1.0.0",
    description: "noxijent api",
