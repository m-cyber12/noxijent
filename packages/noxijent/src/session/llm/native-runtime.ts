...
import {
  ...
  type JsonSchema,
  type LLMEvent,
} from "@noxijent-ai/llm"
import type { LLMClientShape } from "@noxijent-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("noxijent"))
    return { type: "unsupported", reason: "provider is not openai, noxijent, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @noxijent-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains noxijent-owned. The native runtime only adapts
      // the @noxijent-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
