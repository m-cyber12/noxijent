import type { NamedError } from "@noxijent-ai/core/util/error"
...
export const GO_UPSELL_URL = "https://noxijent.ai/go"
...
export function retryable(error: Err, provider: string) {
  ...
  if (SessionV1.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: "Subscribe to Noxijent Go for reliable access to the best open-source models for $10/month.",
          label: "subscribe",
          link: GO_UPSELL_URL,
        },
      }
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = `https://noxijent.ai/workspace/${workspace}/go`
      return {
