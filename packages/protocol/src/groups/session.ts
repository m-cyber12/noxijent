...
import { SessionInput } from "@noxijent-ai/schema/session-input"
import { PromptInput } from "@noxijent-ai/schema/prompt-input"
import { Session } from "@noxijent-ai/schema/session"
import { Project } from "@noxijent-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@noxijent-ai/schema/schema"
import { Workspace } from "@noxijent-ai/schema/workspace"
...
import { Agent } from "@noxijent-ai/schema/agent"
import { Model } from "@noxijent-ai/schema/model"
import { Location } from "@noxijent-ai/schema/location"
import { Revert } from "@noxijent-ai/schema/revert"
import { SessionEvent } from "@noxijent-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Noxijent process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Noxijent process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
