...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("noxijent-find")
  api.delete("noxijent-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("noxijent-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("noxijent-find", new Highlight(...rest))
    return true
  }
