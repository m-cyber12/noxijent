...
import { createNoxijent, type ToolPart } from "@noxijent-ai/sdk"
...
console.log("🚀 Starting noxijent server...")
const noxijent = await createNoxijent({
  port: 0,
})
console.log("✅ Noxijent server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await noxijent.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new noxijent session...")
    const { client, server } = noxijent
    ...
    console.log("✅ Created noxijent session:", createResult.data.id)
  ...
  console.log("📝 Sending to noxijent:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Noxijent response:", JSON.stringify(result, null, 2))
