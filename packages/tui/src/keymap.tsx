...
export const NOXIJENT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const NOXIJENT_MODE_KEY = "noxijent.mode"

export const NoxijentKeymapProvider = KeymapProvider
export const useNoxijentKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type NoxijentModeStack = ReturnType<typeof createNoxijentModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, NoxijentModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createNoxijentModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(NOXIJENT_MODE_KEY, NOXIJENT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(NOXIJENT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(NOXIJENT_MODE_KEY, stack.at(-1)?.mode ?? NOXIJENT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? NOXIJENT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(NOXIJENT_MODE_KEY, undefined)
...
export function useNoxijentModeStack() {
  return getNoxijentModeStack(useNoxijentKeymap())
}

export function getNoxijentModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Noxijent mode stack is not registered for this keymap")
  return value
}
...
export function registerNoxijentKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createNoxijentModeStack(keymap)
