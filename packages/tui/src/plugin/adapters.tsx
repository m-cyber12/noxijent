import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@noxijent-ai/plugin/tui"
...
import type { useNoxijentKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNoxijentKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNoxijentModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNoxijentModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
