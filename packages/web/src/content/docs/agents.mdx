...
There are two types of agents in Noxijent; primary agents and subagents.
...
Noxijent comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Noxijent comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Noxijent comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Noxijent's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `noxijent.json` config file:
...
If no temperature is specified, Noxijent uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Noxijent config and the project specific config.
...
The model ID in your Noxijent config uses the format `provider/model-id`. For example, if you're using [Noxijent Zen](/docs/zen), you would use `noxijent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `noxijent models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/noxijent).
:::
