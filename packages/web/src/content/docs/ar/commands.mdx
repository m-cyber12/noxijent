...
أنشئ الملف `.noxijent/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Noxijent أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Noxijent:
...
- على مستوى النظام: `~/.config/noxijent/commands/`
- لكل مشروع: `.noxijent/commands/`
...
يتضمن noxijent عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
