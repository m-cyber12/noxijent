...
يمكنك إضافة أدوات خارجية إلى Noxijent باستخدام _Model Context Protocol_، أو MCP. يدعم Noxijent خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Noxijent](https://noxijent.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/noxijent` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Noxijent تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Noxijent بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Noxijent المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `noxijent mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Noxijent الرموز بشكل آمن في `~/.local/share/noxijent/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Noxijent إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Noxijent مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Noxijent بحساب Sentry الخاص بك.
