---
title: المزوّدون
description: استخدام أي موفّر LLM في Noxijent.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Noxijent ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Noxijent.
...
في `~/.local/share/noxijent/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Noxijent.
...
Noxijent Zen هي قائمة نماذج يوفّرها فريق Noxijent وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Noxijent. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Noxijent Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر noxijent، ثم انتقل إلى [noxijent.ai/auth](https://noxijent.ai/auth).
...
يعمل مثل أي مزوّد آخر في Noxijent واستخدامه اختياري بالكامل.

---

## Noxijent Go

Noxijent Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Noxijent، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Noxijent.

1. شغّل الأمر `/connect` في TUI، واختر `Noxijent Go`، ثم انتقل إلى [noxijent.ai/auth](https://noxijent.ai/zen).
...
لاستخدام Amazon Bedrock مع Noxijent:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل noxijent:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `noxijent.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Noxijent غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين noxijent لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل noxijent بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات noxijent.
...
تخزين الرمز في مساحة تخزين مصادقة noxijent.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Noxijent نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Noxijent
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`noxijent.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع noxijent:
...
لاستخدام Google Vertex AI مع Noxijent:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر noxijent، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [noxijent-helicone-session](https://github.com/H2Shami/noxijent-helicone-session) لتسجيل كل محادثة Noxijent تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Noxijent مدرجة كجلسة مستقلة.
...
يمكنك ضبط noxijent لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط noxijent لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط noxijent لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Noxijent. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/noxijent) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Noxijent:
...
3. انسخ مفتاح API لاستخدامه في Noxijent.
...
6. **مهم**: قبل استخدام نماذج السحابة في Noxijent، يجب سحب معلومات النموذج محليا:
...
Noxijent Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Noxijent. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Noxijent Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Noxijent Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Noxijent.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات noxijent.
...
5. يمكنك أيضا تخصيصها عبر إعدادات noxijent. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Noxijent:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات noxijent. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع noxijent. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `noxijent.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Noxijent بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `noxijent auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات noxijent وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات noxijent.
