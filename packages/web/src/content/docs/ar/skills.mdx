...
تتيح مهارات الوكيل لـ Noxijent اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Noxijent في هذه المواقع:

- إعدادات المشروع: `.noxijent/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/noxijent/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Noxijent صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.noxijent/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/noxijent/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.noxijent/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Noxijent المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `noxijent.json`:
...
**للوكلاء المدمجين** (في `noxijent.json`):
