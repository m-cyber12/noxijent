...
مع Noxijent يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Noxijent سمتنا `noxijent`.
...
يأتي Noxijent مع عدة سمات مدمجة.
...
- يريدون أن يطابق Noxijent مظهر terminal لديهم
...
يدعم Noxijent نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/noxijent/themes/*.json` أو `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.noxijent/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.noxijent/themes/*.json`
