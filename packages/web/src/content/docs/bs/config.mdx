---
title: Konfiguracija
description: Korištenje Noxijent JSON konfiguracije.
---

Možete konfigurirati Noxijent koristeći JSON konfiguracijski fajl.

---

## Format

Noxijent podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/noxijent`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/noxijent/noxijent.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`NOXIJENT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`noxijent.json` u projektu) - postavke specifične za projekat
5. **`.noxijent` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`NOXIJENT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.noxijent` i `~/.config/noxijent` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/noxijent` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Noxijent konfiguraciju u `~/.config/noxijent/noxijent.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/noxijent/tui.json`.
...
Dodajte `noxijent.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Noxijent pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NOXIJENT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NOXIJENT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.noxijent` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.noxijent`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`noxijent.ai/config.json`**](https://noxijent.ai/config.json).

TUI konfiguracija koristi [**`noxijent.ai/tui.json`**](https://noxijent.ai/tui.json).
...
Koristite `NOXIJENT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `noxijent.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `noxijent serve` i `noxijent web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Noxijent server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `noxijent.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Noxijent konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Noxijent pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/noxijent/agents/` ili `.noxijent/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Noxijent će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`noxijent run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/noxijent/commands/` ili `.noxijent/commands/`. [Saznajte više ovdje](/docs/commands).
...
Noxijent će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Noxijent **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Noxijent sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.noxijent/plugins/` ili `~/.config/noxijent/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Noxijent da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
