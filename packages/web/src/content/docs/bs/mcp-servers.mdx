...
Možete dodati vanjske alate u Noxijent koristeći _Model Context Protocol_, ili MCP. Noxijent podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Noxijent Config](https://noxijent.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/noxijent`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Noxijent automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Noxijent će:
...
Ako server zahtijeva autentifikaciju, Noxijent će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `noxijent mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Noxijent sigurno čuva tokene u `~/.local/share/noxijent/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Noxijent, zajedno s ugrađenim alatima. Možete njima upravljati kroz Noxijent konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Noxijent sa vašim Sentry nalogom.
