---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Noxijent.
---

Dodaci vam omogućavaju da proširite Noxijent spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Noxijent.
...
- `.noxijent/plugins/` - Dodaci na nivou projekta
- `~/.config/noxijent/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/noxijent/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/noxijent/noxijent.json`)
2. Konfiguracija projekta (`noxijent.json`)
3. Globalni direktorij dodataka (`~/.config/noxijent/plugins/`)
4. Direktorij dodataka projekta (`.noxijent/plugins/`)
...
Noxijent pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Noxijent SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Noxijent.
...
Spriječite noxijent da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u noxijent:
...
Pomoćnik `tool` kreira prilagođeni alat koji noxijent može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za noxijent zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://noxijent.ai/docs/sdk) za detalje.
