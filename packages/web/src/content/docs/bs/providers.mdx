---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Noxijent.
---

import config from "../../../../config.mjs"
export const console = config.console

Noxijent koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Noxijent konfiguraciji.
...
u `~/.local/share/noxijent/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Noxijent
config.
...
Noxijent Zen je lista modela koje je obezbedio Noxijent tim koji su bili
testirano i potvrđeno da dobro radi sa Noxijent. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Noxijent Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite noxijent i idite na [noxijent.ai/auth](https://noxijent.ai/auth).
...
Radi kao i svaki drugi provajder u Noxijent i potpuno je opcionalan za korištenje.

---

## Noxijent Go

Noxijent Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Noxijent tim i koji su testirani i verificirani da dobro rade s Noxijent-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Noxijent Go` i idite na [noxijent.ai/auth](https://noxijent.ai/zen).
...
Da biste koristili Amazon Bedrock s Noxijent:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete noxijent:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `noxijent.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Noxijent.
:::
...
Možete konfigurirati noxijent za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi noxijent ispravno radio.
   :::
...
Također možete dodati modele kroz svoju noxijent konfiguraciju.
...
da pohrani token u noxijent auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Noxijent koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Noxijent
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`noxijent.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `noxijent.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s noxijent:
...
Za korištenje Google Vertex AI s Noxijent:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće noxijent.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem noxijenta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [noxijent-helicone-session](https://github.com/H2Shami/noxijent-helicone-session) da automatski prijavite svaki Noxijent razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Noxijent razgovor naveden kao zasebna sesija.
...
Možete konfigurirati noxijent za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati noxijent za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati noxijent za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Noxijent. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/noxijent) za detalje.
:::
...
Da biste koristili Ollama Cloud s Noxijent:
...
3. Kopirajte API ključ za korištenje u Noxijent.
...
6. **Važno**: Prije upotrebe modela oblaka u Noxijent, morate lokalno povući informacije o modelu:
...
Noxijent Zen je lista testiranih i verifikovanih modela koju je obezbedio Noxijent tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Noxijent Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Noxijent Zen**.

```txt
   /connect
```

3. Unesite svoj Noxijent API ključ.
...
Također možete dodati dodatne modele putem vaše noxijent konfiguracije.
...
5. Također ih možete prilagoditi putem vaše noxijent konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Noxijentom:
...
Također možete prilagoditi modele kroz svoju noxijent konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s noxijent-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `noxijent.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Noxijent da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `noxijent auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Noxijent konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj noxijent konfiguraciji.
