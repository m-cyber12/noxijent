---
title: Server
description: Komunicirajte s noxijent serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `noxijent serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji noxijent klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `noxijent.local` |
...
Postavite `NOXIJENT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `noxijent`, ili postavite `NOXIJENT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `noxijent serve` i za `noxijent web`.
...
Kada pokrenete `noxijent`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite noxijent server za programsku interakciju sa noxijent-om.
:::

Ova arhitektura omogucava noxijent podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `noxijent serve` da startate standalone server. Ako je noxijent TUI vec pokrenut, `noxijent serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Noxijent [IDE](/docs/ide) pluginovi.
...
noxijent server izlaže sljedece API-je.
