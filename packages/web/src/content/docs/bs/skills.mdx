...
Agent skills omogucavaju Noxijent da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Noxijent pretrazuje ove lokacije:

- Konfiguracija projekta: `.noxijent/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/noxijent/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Noxijent ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.noxijent/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.noxijent/skills/git-release/SKILL.md` ovako:
...
Noxijent navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `noxijent.json`:
...
**Za ugradene agente** (u `noxijent.json`):
