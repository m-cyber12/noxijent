...
U Noxijent mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Noxijent koristi nasu `noxijent` temu.
...
Noxijent dolazi sa vise ugradenih tema.
...
- Zele da Noxijent odgovara izgledu njihovog terminala
...
Noxijent podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/noxijent/themes/*.json` ili `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.noxijent/themes/*.json`
4. **Trenutni radni direktorij** - `./.noxijent/themes/*.json`
