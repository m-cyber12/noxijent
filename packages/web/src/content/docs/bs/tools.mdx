...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Noxijent dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Noxijent.
...
:::note
Ovaj alat je dostupan samo kada je `NOXIJENT_EXPERIMENTAL_LSP_TOOL=true` (ili `NOXIJENT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Noxijent provajdera ili kada je varijabla `NOXIJENT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Noxijent:
