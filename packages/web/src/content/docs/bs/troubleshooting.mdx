...
Da biste otklonili probleme s Noxijent, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `noxijent --log-level DEBUG`.

---

## Pohrana

Noxijent pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Noxijent Desktop pokreće lokalni Noxijent server (`noxijent-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Noxijent` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/noxijent/noxijent.jsonc` (ili `~/.config/noxijent/noxijent.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/noxijent/noxijent.jsonc`
...
Noxijent također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Noxijent mogao ponovo izgraditi.

1. Potpuno zatvorite Noxijent Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/noxijent`
- **Linux**: obrišite `~/.cache/noxijent` (ili pokrenite `rm -rf ~/.cache/noxijent`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\noxijent`

3. Ponovo pokrenite Noxijent Desktop.

---

### Rješavanje problema sa vezom na serveru

Noxijent Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `noxijent.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `NOXIJENT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `NOXIJENT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Noxijent Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Noxijent karakteristikama.

---

### Obavještenja se ne prikazuju

Noxijent Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Noxijent u postavkama vašeg OS-a, i
...
1. Zatvorite Noxijent Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Noxijent Desktop aplikacije):

- `noxijent.settings.dat` (URL zadanog servera za desktop)
- `noxijent.global.dat` i `noxijent.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `noxijent models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Noxijent dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Noxijent da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Noxijent da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Noxijent će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
