...
Create `.noxijent/commands/test.md`:
...
You can add custom commands through the Noxijent config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Noxijent [config](/docs/config):
...
noxijent includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
