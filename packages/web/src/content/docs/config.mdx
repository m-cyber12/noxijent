---
title: Config
description: Using the Noxijent JSON config.
---

You can configure Noxijent using a JSON config file.

---

## Format

Noxijent supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="noxijent.jsonc"
{
...
1. **Remote config** (from `.well-known/noxijent`) - organizational defaults
2. **Global config** (`~/.config/noxijent/noxijent.json`) - user preferences
3. **Custom config** (`NOXIJENT_CONFIG` env var) - custom overrides
4. **Project config** (`noxijent.json` in project) - project-specific settings
5. **`.noxijent` directories** - agents, commands, plugins
6. **Inline config** (`NOXIJENT_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/noxijent/` on macOS) - admin-controlled
...
:::note
The `.noxijent` and `~/.config/noxijent` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/noxijent` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/noxijent"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Noxijent config in `~/.config/noxijent/noxijent.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/noxijent/tui.json`.
...
Add `noxijent.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Noxijent starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `NOXIJENT_CONFIG` environment variable.

```bash
export NOXIJENT_CONFIG=/path/to/my/custom-config.json
noxijent run "Hello world"
```
...
Specify a custom config directory using the `NOXIJENT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.noxijent` directory, and should
follow the same structure.

```bash
export NOXIJENT_CONFIG_DIR=/path/to/my/config-directory
noxijent run "Hello world"
```

The custom directory is loaded after the global config and `.noxijent` directories, so it **can override** their settings.
...
Drop an `noxijent.json` or `noxijent.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/noxijent/` |
| Linux    | `/etc/noxijent/`                         |
| Windows  | `%ProgramData%\noxijent`                 |
...
On macOS, Noxijent reads managed preferences from the `ai.noxijent.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Noxijent checks these paths:

1. `/Library/Managed Preferences/<user>/ai.noxijent.managed.plist`
2. `/Library/Managed Preferences/ai.noxijent.managed.plist`

The plist keys map directly to `noxijent.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.noxijent.managed` PayloadType. The Noxijent config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.noxijent.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.noxijent.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.noxijent</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`noxijent.ai/config.json`**](https://noxijent.ai/config.json).

TUI config uses [**`noxijent.ai/tui.json`**](https://noxijent.ai/tui.json).
...
Use `NOXIJENT_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `noxijent.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `noxijent serve` and `noxijent web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Noxijent server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `noxijent.local`. Useful for running multiple instances on the same network.
...
If not specified, Noxijent will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Noxijent config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Noxijent tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Noxijent actions on configured resources. Currently, policies can control which providers Noxijent may use.
...
Noxijent normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Noxijent omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/noxijent/agents/` or `.noxijent/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Noxijent will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`noxijent run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/noxijent/commands/` or `.noxijent/commands/`. [Learn more here](/docs/commands).
...
Noxijent uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Noxijent will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, noxijent **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Noxijent with custom tools, hooks, and integrations.

Place plugin files in `.noxijent/plugins/` or `~/.config/noxijent/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="noxijent.json"
{
  "$schema": "https://noxijent.ai/config.json",
  "plugin": ["noxijent-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Noxijent to only use specific providers rather than disabling them one by one.
