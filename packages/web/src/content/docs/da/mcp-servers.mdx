...
Du kan tilføje eksterne værktøjer til Noxijent ved hjælp af _Model Context Protocol_ eller MCP. Noxijent understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Noxijent Config](https://noxijent.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/noxijent`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Noxijent håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Noxijent:
...
Hvis serveren kræver godkendelse, vil Noxijent bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `noxijent mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Noxijent opbevare tokens sikkert i `~/.local/share/noxijent/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Noxijent-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Noxijent til din Sentry-konto.
