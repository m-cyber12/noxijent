---
title: Plugins
description: Skriv dine egne plugins for at udvide Noxijent.
---

Plugins giver dig mulighed for at udvide Noxijent ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Noxijents standardadfærd.
...
- `.noxijent/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/noxijent/node_modules/`.
...
1. Global konfiguration (`~/.config/noxijent/noxijent.json`)
2. Projektkonfiguration (`noxijent.json`)
3. Global plugin-mappe (`~/.config/noxijent/plugins/`)
4. Projekt plugin bibliotek (`.noxijent/plugins/`)
...
Noxijent kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En noxijent SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide noxijent.
...
:::note
Hvis du bruger Noxijent desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå noxijent i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til noxijent:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som noxijent kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for noxijent sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://noxijent.ai/docs/sdk) for detaljer.
