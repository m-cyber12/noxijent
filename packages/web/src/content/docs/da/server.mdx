---
title: Server
description: Interager med noxijent-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `noxijent serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en noxijent-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `noxijent.local` |
...
Indstil `NOXIJENT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `noxijent`, eller indstil `NOXIJENT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `noxijent serve` og `noxijent web`.
...
Når du kører `noxijent` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug noxijent-serveren til at interagere med noxijent programmatisk.
:::

Denne arkitektur lader noxijent understøtte flere klienter og lader dig interagere med noxijent programmatisk.

Du kan køre `noxijent serve` for at starte en selvstændig server. Hvis du har
noxijent TUI kørende, vil `noxijent serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Noxijent [IDE](/docs/ide) plugins.
...
Noxijent-serveren viser følgende API'er.
