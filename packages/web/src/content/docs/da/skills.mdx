...
Agentferdigheter lar Noxijent oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Noxijent søker etter disse stedene:

- Prosjektkonfigurasjon: `.noxijent/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/noxijent/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Noxijent opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.noxijent/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.noxijent/skills/git-release/SKILL.md` slik:
...
Noxijent viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `noxijent.json`:
...
**For innebygde agenter** (i `noxijent.json`):
