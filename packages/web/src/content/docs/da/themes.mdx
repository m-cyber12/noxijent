...
Med Noxijent kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Noxijent vårt eget `noxijent`-tema.
...
Noxijent støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/noxijent/themes/*.json` eller `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.noxijent/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.noxijent/themes/*.json`
