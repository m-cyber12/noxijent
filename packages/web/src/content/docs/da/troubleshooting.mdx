...
For at fejlfinde problemer med Noxijent, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `noxijent --log-level DEBUG`.

---

## Lagring

noxijent gemmer sessionsdata og andre applikationsdata på disken på:
...
Noxijent Desktop kører en lokal Noxijent-server (`noxijent-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Noxijent`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/noxijent/noxijent.jsonc` (eller `~/.config/noxijent/noxijent.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/noxijent/noxijent.jsonc`
...
Noxijent kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Noxijent kan genopbygge den.

1. Afslut Noxijent Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/noxijent`
- **Linux**: slet `~/.cache/noxijent` (eller kør `rm -rf ~/.cache/noxijent`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\noxijent`

3. Start Noxijent Desktop på ny.

---

### Løs problemer med serverforbindelse

Noxijent Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `noxijent.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `NOXIJENT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NOXIJENT_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Noxijent Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Noxijent.

---

### Meddelelser vises ikke

Noxijent Desktop viser kun systemvarsler når:

- varsler er aktiveret for Noxijent i dine OS-indstillinger, og
...
1. Afslut Noxijent Desktop.
2. Find og slet disse filer (de findes i Noxijent Desktop-appens datamappe):

- `noxijent.settings.dat` (skrivebordsstandardserver URL)
- `noxijent.global.dat` og `noxijent.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Noxijent:
...
For at finde ud af hvilke modeller du har adgang til, kør `noxijent models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. noxijent installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start noxijent på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge noxijent til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
noxijent vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
