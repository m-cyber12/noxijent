...
Erstellen Sie `.noxijent/commands/test.md`:

```md title=".noxijent/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Noxijent-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Noxijent [config](/docs/config):

```json title="noxijent.jsonc" {4-12}
{
...
- Global: `~/.config/noxijent/commands/`
- Pro Projekt: `.noxijent/commands/`

```markdown title="~/.config/noxijent/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".noxijent/commands/component.md"
---
...
```md title=".noxijent/commands/create-file.md"
---
...
```md title=".noxijent/commands/analyze-coverage.md"
---
...
```md title=".noxijent/commands/review-changes.md"
---
...
```md title=".noxijent/commands/review-component.md"
---
...
Noxijent enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
