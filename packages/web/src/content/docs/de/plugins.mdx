---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Noxijent zu erweitern.
---

Mit Plugins können Sie Noxijent erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Noxijent zu ändern.
...
- `.noxijent/plugins/` – Plugins auf Projektebene
- `~/.config/noxijent/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/noxijent/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/noxijent/noxijent.json`)
2. Projektkonfiguration (`noxijent.json`)
3. Globales Plugin-Verzeichnis (`~/.config/noxijent/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.noxijent/plugins/`)
...
Noxijent führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Noxijent-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Noxijent erweitern können.
...
:::note
Wenn Sie die Noxijent-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Noxijent `.env`-Dateien liest:
...
Plugins können Noxijent auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Noxijent aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Noxijent zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://noxijent.ai/docs/sdk).
