---
title: Regeln
description: Lege eigene Anweisungen fuer noxijent fest.
---

Du kannst noxijent ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in noxijent den Befehl `/init` aus.
...
So kann noxijent sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht noxijent sie zu erweitern.
...
noxijent kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/noxijent/AGENTS.md` speichern.
Diese gelten in allen noxijent-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Noxijent dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/noxijent/AGENTS.md` existiert)
...
Beim Start sucht noxijent in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/noxijent/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `noxijent.json` oder in `~/.config/noxijent/noxijent.json` angeben.
...
noxijent parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit noxijent.json

Empfohlen ist das `instructions`-Feld in der `noxijent.json`:
...
Du kannst noxijent explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `noxijent.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
