---
title: Server
description: Nutze den noxijent-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `noxijent serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein noxijent-Client nutzen kann.
...
Setze `NOXIJENT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `noxijent`, kann aber mit `NOXIJENT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `noxijent serve` und `noxijent web`.
...
Wenn du `noxijent` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den noxijent-Server, um noxijent programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `noxijent serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `noxijent serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Noxijent-[IDE](/docs/ide)-Plugins.
...
Der noxijent-Server stellt folgende APIs bereit.
