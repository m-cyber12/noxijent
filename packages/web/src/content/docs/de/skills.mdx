...
Agent Skills erlauben Noxijent, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Noxijent sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Noxijent vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.noxijent/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.noxijent/skills/git-release/SKILL.md` zum Beispiel so:
...
Noxijent listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `noxijent.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `noxijent.json`):
