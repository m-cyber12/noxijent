...
Mit Noxijent kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Noxijent das `noxijent`-Theme.
...
Noxijent liefert mehrere eingebaute Themes mit.
...
- Noxijent optisch ans Terminal anpassen wollen
...
Noxijent unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/noxijent/themes/*.json` oder `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.noxijent/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.noxijent/themes/*.json`
