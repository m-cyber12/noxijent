...
Wenn Noxijent Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `noxijent --log-level DEBUG`.

---

## Speicher

noxijent speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Noxijent Desktop startet im Hintergrund einen lokalen Noxijent-Server (`noxijent-cli`-Sidecar).
...
- Nur macOS: `Noxijent`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/noxijent/noxijent.jsonc` (oder `~/.config/noxijent/noxijent.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/noxijent/noxijent.jsonc`
...
Noxijent kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Noxijent ihn neu aufbauen kann.

1. Beende Noxijent Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/noxijent`
- **Linux**: loesche `~/.cache/noxijent` (oder fuehre aus: `rm -rf ~/.cache/noxijent`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\noxijent`

3. Starte Noxijent Desktop neu.

---

### Server-Verbindungsprobleme beheben

Noxijent Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `noxijent.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `NOXIJENT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `NOXIJENT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Noxijent Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Noxijent-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Noxijent Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Noxijent in den OS-Einstellungen aktiviert sind, und
...
1. Beende Noxijent Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Noxijent Desktop):

- `noxijent.settings.dat` (desktop default server URL)
- `noxijent.global.dat` und `noxijent.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Noxijent hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `noxijent models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. noxijent installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte noxijent neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt noxijent, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
noxijent erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
