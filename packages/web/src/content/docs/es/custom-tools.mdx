...
description: Cree herramientas que LLM pueda llamar en noxijent.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de noxijent como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.noxijent/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/noxijent/tools/`.
...
```ts title=".noxijent/tools/database.ts" {1}
...
```ts title=".noxijent/tools/math.ts"
import { tool } from "@noxijent-ai/plugin"
...
```ts title=".noxijent/tools/bash.ts"
import { tool } from "@noxijent-ai/plugin"
...
```ts title=".noxijent/tools/project.ts" {8}
import { tool } from "@noxijent-ai/plugin"
...
```python title=".noxijent/tools/add.py"
import sys
...
```ts title=".noxijent/tools/python-add.ts" {10}
import { tool } from "@noxijent-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".noxijent/tools/add.py")
