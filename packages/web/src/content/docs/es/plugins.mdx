---
title: Complementos
description: Escriba sus propios complementos para extender Noxijent.
---

Los complementos le permiten extender Noxijent al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Noxijent.
...
- `.noxijent/plugins/` - Complementos a nivel de proyecto
- `~/.config/noxijent/plugins/` - Complementos globales
...
```json title="noxijent.json"
{
  ...
  "plugin": ["noxijent-helicone-session", "noxijent-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/noxijent/node_modules/`.
...
1. Configuración global (`~/.config/noxijent/noxijent.json`)
2. Configuración del proyecto (`noxijent.json`)
3. Directorio global de complementos (`~/.config/noxijent/plugins/`)
4. Directorio de complementos del proyecto (`.noxijent/plugins/`)
...
```json title=".noxijent/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Noxijent ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".noxijent/plugins/my-plugin.ts"
...
```js title=".noxijent/plugins/example.js"
...
- `client`: Un cliente SDK noxijent para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar noxijent.
...
```js title=".noxijent/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "noxijent"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Noxijent, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que noxijent lea archivos `.env`:

```javascript title=".noxijent/plugins/env-protection.js"
...
```javascript title=".noxijent/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a noxijent:

```ts title=".noxijent/plugins/custom-tools.ts"
import { type Plugin, tool } from "@noxijent-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que noxijent puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para noxijent junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://noxijent.ai/docs/sdk) para obtener más detalles.
...
```ts title=".noxijent/plugins/compaction.ts"
import type { Plugin } from "@noxijent-ai/plugin"
...
```ts title=".noxijent/plugins/custom-compaction.ts"
import type { Plugin } from "@noxijent-ai/plugin"
