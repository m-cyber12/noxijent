---
title: Servidor
description: Interactuar con el servidor noxijent a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `noxijent serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente noxijent puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `noxijent.local` |
...
Configure `NOXIJENT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `noxijent`, o configure `NOXIJENT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `noxijent serve` como a `noxijent web`.
...
Cuando ejecuta `noxijent`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor noxijent para interactuar con noxijent mediante programación.
:::

Esta arquitectura permite que noxijent admita múltiples clientes y le permite interactuar con noxijent mediante programación.

Puede ejecutar `noxijent serve` para iniciar un servidor independiente. Si tienes el
noxijent TUI ejecutándose, `noxijent serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Noxijent [IDE](/docs/ide).
...
El servidor noxijent expone las siguientes API.
