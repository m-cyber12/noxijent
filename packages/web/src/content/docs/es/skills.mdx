...
Las habilidades del agente permiten a Noxijent descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Noxijent busca estas ubicaciones:

- Configuración del proyecto: `.noxijent/skills/<name>/SKILL.md`
- Configuración global: `~/.config/noxijent/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Noxijent sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.noxijent/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.noxijent/skills/git-release/SKILL.md` así:
...
Noxijent enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `noxijent.json`:
...
**Para agentes integrados** (en `noxijent.json`):
