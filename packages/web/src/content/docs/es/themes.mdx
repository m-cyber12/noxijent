...
Con Noxijent puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Noxijent usa nuestro propio tema `noxijent`.
...
Noxijent viene con varios temas integrados.
...
- Quiere que Noxijent coincida con la apariencia de su terminal
...
Noxijent admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/noxijent/themes/*.json` o `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.noxijent/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.noxijent/themes/*.json`
