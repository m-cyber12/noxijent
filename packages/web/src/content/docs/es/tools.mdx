...
Las herramientas permiten que LLM realice acciones en su código base. Noxijent viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Noxijent.
...
:::note
Esta herramienta solo está disponible cuando `NOXIJENT_EXPERIMENTAL_LSP_TOOL=true` (o `NOXIJENT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Noxijent o cuando la variable de entorno `NOXIJENT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Noxijent:

```bash
NOXIJENT_ENABLE_EXA=1 noxijent
```
