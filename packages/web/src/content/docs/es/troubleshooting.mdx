...
Para depurar problemas con Noxijent, comience verificando los registros y los datos locales que almacena en el disco.
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `noxijent --log-level DEBUG`.

---

## Almacenamiento

noxijent almacena datos de sesión y otros datos de aplicaciones en el disco en:
...
Noxijent Desktop ejecuta un servidor Noxijent local (el sidecar `noxijent-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Noxijent` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/noxijent/noxijent.jsonc` (o `~/.config/noxijent/noxijent.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/noxijent/noxijent.jsonc`
...
Noxijent también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Noxijent pueda reconstruirlo.

1. Salga de Noxijent Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/noxijent`
- **Linux**: eliminar `~/.cache/noxijent` (o ejecutar `rm -rf ~/.cache/noxijent`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\noxijent`

3. Reinicie el escritorio Noxijent.

---

### Solucionar problemas de conexión del servidor

Noxijent Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `noxijent.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `NOXIJENT_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `NOXIJENT_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Noxijent requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Noxijent.

---

### Notificaciones que no se muestran

Noxijent Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Noxijent en la configuración de su sistema operativo, y
...
1. Salga del escritorio Noxijent.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Noxijent):

- `noxijent.settings.dat` (URL del servidor predeterminado de escritorio)
- `noxijent.global.dat` y `noxijent.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Noxijent:
...
3. Asegúrese de tener la última versión con `noxijent upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `noxijent models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. noxijent instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie noxijent para reinstalar los paquetes del proveedor más recientes.

Esto obligará a noxijent a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
noxijent detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
