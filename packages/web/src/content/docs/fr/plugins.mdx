---
title: Plugins
description: Écrivez vos propres plugins pour étendre Noxijent.
---

Les plugins vous permettent d'étendre Noxijent en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Noxijent.
...
- `.noxijent/plugins/` - Plugins au niveau du projet
- `~/.config/noxijent/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/noxijent/node_modules/`.
...
1. Configuration globale (`~/.config/noxijent/noxijent.json`)
2. Configuration du projet (`noxijent.json`)
3. Répertoire global des plugins (`~/.config/noxijent/plugins/`)
4. Répertoire des plugins du projet (`.noxijent/plugins/`)
...
Noxijent exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK noxijent pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre noxijent.
...
:::note
Si vous utilisez l'application de bureau Noxijent, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez noxijent de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à noxijent :
...
L'assistant `tool` crée un outil personnalisé que noxijent peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour noxijent aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://noxijent.ai/docs/sdk) pour plus de détails.
