---
title: SDK
description: Client JS de type sécurisé pour le serveur noxijent.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK noxijent JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler noxijent par programme.
...
Créez une instance de noxijent :

```javascript
import { createNoxijent } from "@noxijent-ai/sdk"

const { client } = await createNoxijent()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `noxijent.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${noxijent.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de noxijent, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@noxijent-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*noxijent" },
})
