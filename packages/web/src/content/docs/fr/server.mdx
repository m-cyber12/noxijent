---
title: Serveur
description: Interagissez avec le serveur noxijent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `noxijent serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client noxijent peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `noxijent.local` |
...
Définissez `NOXIJENT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `noxijent`, ou définissez `NOXIJENT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `noxijent serve` et à `noxijent web`.
...
Lorsque vous exécutez `noxijent`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur noxijent pour interagir avec noxijent par programmation.
:::

Cette architecture permet à noxijent de prendre en charge plusieurs clients et vous permet d'interagir avec noxijent par programme.

Vous pouvez exécuter `noxijent serve` pour démarrer un serveur autonome. Si vous avez le
noxijent TUI en cours d'exécution, `noxijent serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Noxijent [IDE](/docs/ide).
...
Le serveur noxijent expose les API suivantes.
