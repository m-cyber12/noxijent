...
Avec Noxijent, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Noxijent utilise notre propre thème `noxijent`.
...
Noxijent est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Noxijent corresponde à l'apparence de leur terminal
...
Noxijent prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/noxijent/themes/*.json` ou `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.noxijent/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.noxijent/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/noxijent/themes
vim ~/.config/noxijent/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .noxijent/themes
vim .noxijent/themes/my-theme.json
```
...
{
  "$schema": "https://noxijent.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
