...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Noxijent est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Noxijent.
...
:::note
Cet outil n'est disponible que lorsque `NOXIJENT_EXPERIMENTAL_LSP_TOOL=true` (ou `NOXIJENT_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Noxijent ou lorsque la variable d'environnement `NOXIJENT_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Noxijent :
