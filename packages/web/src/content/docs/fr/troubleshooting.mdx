...
Pour déboguer les problèmes avec Noxijent, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `noxijent --log-level DEBUG`.

---

## Stockage

noxijent stocke les données de session et autres données d'application sur le disque à l'emplacement :
...
Noxijent Desktop exécute un serveur Noxijent local (le side-car `noxijent-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Noxijent` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/noxijent/noxijent.jsonc`
...
Noxijent peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Noxijent puisse le reconstruire.

1. Quittez complètement Noxijent Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/noxijent`
- **Linux** : supprimez `~/.cache/noxijent` (ou exécutez `rm -rf ~/.cache/noxijent`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\noxijent`

3. Redémarrez le bureau Noxijent.

---

### Résoudre les problèmes de connexion au serveur

Noxijent Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `noxijent.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `NOXIJENT_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `NOXIJENT_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Noxijent Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Noxijent.

---

### Les notifications ne s'affichent pas

Noxijent Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Noxijent dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Noxijent.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Noxijent Desktop) :

- `noxijent.settings.dat` (serveur de bureau par défaut URL)
- `noxijent.global.dat` et `noxijent.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Noxijent :
...
3. Assurez-vous d'avoir la dernière version avec `noxijent upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `noxijent models`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. noxijent installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
2. Redémarrez noxijent pour réinstaller les derniers packages du fournisseur

Cela forcera noxijent à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
noxijent détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
