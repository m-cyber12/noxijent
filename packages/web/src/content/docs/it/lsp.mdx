---
title: Server LSP
description: Noxijent si integra con i tuoi server LSP.
---

Noxijent può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Noxijent include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `NOXIJENT_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e noxijent apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di noxijent.
