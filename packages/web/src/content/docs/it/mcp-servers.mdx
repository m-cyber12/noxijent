...
Puoi aggiungere strumenti esterni a Noxijent usando il _Model Context Protocol_, o MCP. Noxijent supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Noxijent Config](https://noxijent.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/noxijent`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Noxijent gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Noxijent:
...
Se il server richiede autenticazione, Noxijent ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `noxijent mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Noxijent memorizzera' i token in modo sicuro in `~/.local/share/noxijent/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Noxijent insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Noxijent come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Noxijent al tuo account Sentry.
