---
title: Plugin
description: Scrivi plugin per estendere Noxijent.
---

I plugin ti permettono di estendere Noxijent agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Noxijent.
...
- `.noxijent/plugins/` - plugin a livello progetto
- `~/.config/noxijent/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/noxijent/node_modules/`.
...
1. Config globale (`~/.config/noxijent/noxijent.json`)
2. Config di progetto (`noxijent.json`)
3. Directory plugin globale (`~/.config/noxijent/plugins/`)
4. Directory plugin di progetto (`.noxijent/plugins/`)
...
Noxijent esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di noxijent per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere noxijent.
...
:::note
Se usi l'app desktop di Noxijent, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a noxijent di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a noxijent:
...
L'helper `tool` crea uno strumento personalizzato che noxijent può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in noxijent insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://noxijent.ai/docs/sdk) per i dettagli.
