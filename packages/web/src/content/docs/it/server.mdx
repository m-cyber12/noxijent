---
title: Server
description: Interagisci con il server noxijent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `noxijent serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client noxijent.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `noxijent.local` |
...
Imposta `NOXIJENT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `noxijent`, oppure imposta `NOXIJENT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `noxijent serve` sia per `noxijent web`.
...
Quando esegui `noxijent` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server noxijent per interagire con noxijent in modo programmatico.
:::

Questa architettura permette a noxijent di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `noxijent serve` per avviare un server standalone. Se la TUI di noxijent e gia in esecuzione, `noxijent serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Noxijent.
...
Il server noxijent espone le seguenti API.
