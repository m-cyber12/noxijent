...
Le skill degli agenti permettono a Noxijent di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Noxijent cerca in queste posizioni:

- Config di progetto: `.noxijent/skills/<name>/SKILL.md`
- Config globale: `~/.config/noxijent/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Noxijent risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.noxijent/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.noxijent/skills/git-release/SKILL.md` cosi':
...
Noxijent elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `noxijent.json`:
...
**Per agenti integrati** (in `noxijent.json`):
