...
Con Noxijent puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Noxijent usa il tema `noxijent`.
...
Noxijent include diversi temi integrati.
...
- Vuole che Noxijent corrisponda all'aspetto del terminale
...
Noxijent supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/noxijent/themes/*.json` o `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.noxijent/themes/*.json`
4. **Directory di lavoro corrente** - in `./.noxijent/themes/*.json`
