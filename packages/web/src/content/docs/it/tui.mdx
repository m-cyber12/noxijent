---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Noxijent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Noxijent fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Noxijent avvia la TUI per la directory corrente.
...
Quando usi la TUI di Noxijent, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Noxijent. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Noxijent. _Alias_: `/quit`, `/q`
...
Questo è separato da `noxijent.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Noxijent attende dopo il leader key. Default: `2000`.
...
Usa `NOXIJENT_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `noxijent.default`.
