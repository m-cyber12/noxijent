...
You can add external tools to Noxijent using the _Model Context Protocol_, or MCP. Noxijent supports both local and remote servers.
...
You can define MCP servers in your [Noxijent Config](https://noxijent.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/noxijent` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Noxijent automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Noxijent will:
...
If the server requires authentication, Noxijent will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `noxijent mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Noxijent will store the tokens securely in `~/.local/share/noxijent/mcp-auth.json`.
...
Your MCPs are available as tools in Noxijent, alongside built-in tools. So you can manage them through the Noxijent config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Noxijent to your Sentry account.
