...
Det er to typer agenter i Noxijent; primære agenter og underagenter.
...
Noxijent kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Noxijent kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Noxijent kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `noxijent.json`:
...
Hvis ingen temperatur er spesifisert, bruker Noxijent modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Noxijent-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Noxijent-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Noxijent Zen](/docs/zen), vil du bruke `noxijent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `noxijent models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/noxijent).
:::
