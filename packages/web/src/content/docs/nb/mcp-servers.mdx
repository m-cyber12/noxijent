...
Du kan legge til eksterne verktøy i Noxijent ved å bruke _Model Context Protocol_ eller MCP. Noxijent støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Noxijent Config](https://noxijent.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="noxijent.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/noxijent`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Noxijent håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Noxijent:
...
Hvis serveren krever autentisering, vil Noxijent be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `noxijent mcp auth <server-name>`.
...
```bash
noxijent mcp auth my-oauth-server
```
...
```bash
noxijent mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Noxijent lagre tokens sikkert i `~/.local/share/noxijent/mcp-auth.json`.
...
noxijent mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Noxijent, sammen med innebygde verktøy. Så du kan administrere dem gjennom Noxijent-konfigurasjonen som alle andre verktøy.
...
```json title="noxijent.json" {11, 14-18}
{
  "$schema": "https://noxijent.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
noxijent mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Noxijent til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Noxijent-samtale oppført som en separat økt.
