...
description: Skriv dine egne programtillegg for å utvide Noxijent.
---

Plugins lar deg utvide Noxijent ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Noxijent.
...
- `.noxijent/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/noxijent/node_modules/`.
...
1. Global konfigurasjon (`~/.config/noxijent/noxijent.json`)
2. Prosjektkonfigurasjon (`noxijent.json`)
3. Global plugin-katalog (`~/.config/noxijent/plugins/`)
4. Prosjektpluginkatalog (`.noxijent/plugins/`)
...
Noxijent kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Noxijent SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Noxijent.
...
:::note
Hvis du bruker Noxijent-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Noxijent fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Noxijent:
...
`tool`-hjelperen lager et tilpasset verktøy som Noxijent kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Noxijent sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://noxijent.ai/docs/sdk) for detaljer.
