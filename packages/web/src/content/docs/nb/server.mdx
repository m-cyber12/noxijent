---
title: Tjener
description: Samhandle med Noxijent-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `noxijent serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Noxijent-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `noxijent.local` |
...
Sett `NOXIJENT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `noxijent` som standard, eller sett `NOXIJENT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `noxijent serve` og `noxijent web`.
...
Når du kjører `noxijent` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Noxijent-serveren til å samhandle med Noxijent programmatisk.
:::

Denne arkitekturen lar Noxijent støtte flere klienter og lar deg samhandle med Noxijent programmatisk.

Du kan kjøre `noxijent serve` for å starte en frittstående server. Hvis du har
Noxijent TUI kjører, vil `noxijent serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Noxijent [IDE](/docs/ide) plugins.
...
noxijent-serveren viser følgende APIer.
