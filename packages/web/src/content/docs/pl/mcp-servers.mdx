...
Możesz dodać dodatkowe informacje do noxijent za pomocą _Model Context Protocol_, w skrócie MCP. noxijent obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [noxijent Config](https://noxijent.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/noxijent`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
noxijent automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, noxijent:
...
Jeśli serwer wymaga uwierzytelnienia, noxijent poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `noxijent mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji noxijent bezpieczne przechowa tokeny w `~/.local/share/noxijent/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w noxijent, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie noxijent, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć noxijent z kontem Sentry.
