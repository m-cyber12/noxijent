---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć noxijent.
---

Wtyczki umożliwiające wyłączenie noxijent poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania noxijent.
...
- `.noxijent/plugins/` - Wtyczki na poziomie projektu
- `~/.config/noxijent/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/noxijent/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/noxijent/noxijent.json`)
2. Konfiguracja projektu (`noxijent.json`)
3. Globalny katalog wtyczek (`~/.config/noxijent/plugins/`)
4. Katalog wtyczek projektu (`.noxijent/plugins/`)
...
noxijent uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z noxijent do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do noxijent.
...
:::note
Jeśli korzystasz z aplikacji komputerowej noxijent, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj noxijent czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do noxijent:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod noxijent. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://noxijent.ai/docs/sdk).
