---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w noxijent.
---

import config from "../../../../config.mjs"
export const console = config.console

noxijent używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji noxijent.
...
w `~/.local/share/noxijent/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim noxijent
config.
...
Noxijent Zen to lista modeli dostarczonych przez zespół noxijent, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z noxijent. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Noxijent Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Noxijent Zen` i przejdź do [noxijent.ai/auth](https://noxijent.ai/zen).
...
Działa jak każdy inny dostawca w noxijent i jest całkowicie opcjonalny w użyciu.

---

## Noxijent Go

Noxijent Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół noxijent, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z noxijent.

1. Uruchom polecenie `/connect` w TUI, wybierz `Noxijent Go` i przejdź do [noxijent.ai/auth](https://noxijent.ai/zen).
...
Aby używać Amazon Bedrock z noxijent:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania noxijent:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `noxijent.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w noxijent nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować noxijent do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod noxijent działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji noxijent.
...
noxijent używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować noxijent
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`noxijent.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem noxijent:
...
Aby używać Google Vertex AI z noxijent:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania noxijent.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą noxijent, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [noxijent-helicone-session](https://github.com/H2Shami/noxijent-helicone-session), aby automatycznie rejestrować każdą konwersację noxijent jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja noxijent będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować noxijent tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować noxijent tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować noxijent tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem noxijent. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/noxijent).
:::
...
Aby korzystać z Ollama Cloud z noxijent:
...
3. Skopiuj klucz API do użycia w noxijent.
...
6. **Ważne**: Przed użyciem modeli chmurowych w noxijent musisz pobrać informacje o modelu lokalnie:
...
Noxijent Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół noxijent. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Noxijent Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Noxijent Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API noxijent.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji noxijent.
...
5. Możesz je także dostosować za pomocą konfiguracji noxijent. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z noxijent:
...
Możesz także dostosować modele za pomocą konfiguracji noxijent. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z noxijent. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `noxijent.json` w katalogu projektu:
...
Pola `limit` pozwalają noxijent zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `noxijent auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację noxijent i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji noxijent.
