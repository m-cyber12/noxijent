...
Umiejętności agenta pozwalają noxijent odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
noxijent przeszukuje te lokalizacje:

- Project config: `.noxijent/skills/<name>/SKILL.md`
- Global config: `~/.config/noxijent/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu noxijent przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.noxijent/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.noxijent/skills/git-release/SKILL.md` w ten sposób:
...
noxijent wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `noxijent.json`:
...
**Dla agentów wbudowanych** (w `noxijent.json`):
