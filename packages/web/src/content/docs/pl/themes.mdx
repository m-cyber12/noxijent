...
Dzięki noxijent możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie noxijent używa naszego własnego motywu `noxijent`.
...
noxijent ma kilka wbudowanych motywów.
...
- Chcą, aby Noxijent pasował do wyglądu ich terminala
...
noxijent obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/noxijent/themes/*.json` lub `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.noxijent/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.noxijent/themes/*.json`
