...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Noxijent posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Noxijent.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `NOXIJENT_EXPERIMENTAL_LSP_TOOL=true` (lub `NOXIJENT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Noxijent lub gdy zmienna środowiskowa `NOXIJENT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Noxijent:
