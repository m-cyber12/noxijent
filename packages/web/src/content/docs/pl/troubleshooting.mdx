...
Aby debugować problemy z noxijent, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `noxijent --log-level DEBUG`.

---

## Przechowywanie danych

noxijent przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
noxijent Desktop uruchamia w tle lokalny serwer noxijent (sidecar `noxijent-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Noxijent` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/noxijent/noxijent.jsonc` (lub `~/.config/noxijent/noxijent.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/noxijent/noxijent.jsonc`
...
noxijent może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby noxijent mógł ją odbudować.

1. Całkowicie zamknij noxijent Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/noxijent`
- **Linux**: usuń `~/.cache/noxijent` (lub uruchom `rm -rf ~/.cache/noxijent`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\noxijent`

3. Uruchom ponownie noxijent Desktop.

---

### Napraw problemy z połączeniem z serwerem

noxijent Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `noxijent.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `NOXIJENT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `NOXIJENT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows noxijent Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami noxijent.

---

### Brak powiadomień

noxijent Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla noxijent w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie noxijent Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji noxijent Desktop):

- `noxijent.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `noxijent.global.dat` i `noxijent.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z noxijent:
...
3. Upewnij się, że masz najnowszą wersję z `noxijent upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `noxijent models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. noxijent dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod noxijent, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to noxijent do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
noxijent wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
