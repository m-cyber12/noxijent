...
Existem dois tipos de agentes no noxijent; agentes primários e subagentes.
...
noxijent vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Noxijent vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Noxijent vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Noxijent, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `noxijent.json`:
...
- Global: `~/.config/noxijent/agents/`
- Por projeto: `.noxijent/agents/`

```markdown title="~/.config/noxijent/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o noxijent usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do noxijent quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do noxijent usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Noxijent Zen](/docs/zen), você usaria `noxijent/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="noxijent.json" {3-6,9-12}
{
  "$schema": "https://noxijent.ai/config.json",
  "tools": {
    "write": true,
...
```json title="noxijent.json" {3-5,8-10}
{
  "$schema": "https://noxijent.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `noxijent models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/noxijent).
:::
...
```markdown title="~/.config/noxijent/agents/docs-writer.md"
---
...
```markdown title="~/.config/noxijent/agents/security-auditor.md"
---
