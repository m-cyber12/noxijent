...
O Noxijent Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Noxijent. Você assina o Noxijent Go e
...
O Noxijent Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O Noxijent Go funciona como qualquer outro provedor no Noxijent.

1. Você entra no **<a href={console}>Noxijent Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Noxijent Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Noxijent Go.
:::
...
- **Hy4 preview**
- **Hy3**
- **Union Alpha Free** (por tempo limitado)
...
O Noxijent Go foi projetado para o [Noxijent](https://noxijent.ai) e outros agentes de programação
...
3. Enviar um ID de sessão estável em `x-noxijent-session` para cada conversa, para que possamos otimizar o roteamento e
   o cache de prompts.

### Clientes validados

Além do Noxijent, os clientes a seguir foram validados para funcionar corretamente
com o Noxijent Go. No entanto, não garantimos que continuarão funcionando no futuro.

| Cliente           | Suporte a sessões                                                                                                                                                                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | As builds que contêm o [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) enviam o header nas requisições principais e auxiliares do Noxijent. A correção foi incorporada após a v0.21.0; essa versão por si só não a inclui.                     |
...
| **ZCode**         | O Go reconhece seu header de sessão nativo. Nossa [solicitação de `x-noxijent-session`](https://github.com/zai-org/feedback/issues/492) continua aberta, mas não é mais necessário enviar especificamente esse header.                                                |
| **Pi**            | As builds atuais enviam informações de sessão para o Noxijent. Atualize instalações mais antigas.                                                                                                                                                                     |
...
| **Kilo Code CLI** | As builds que contêm o [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restauram os headers de sessão do Noxijent. Essa correção abrange a CLI, não a extensão do VS Code. Consulte a [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
| Union Alpha Free                        | Free    | Free   | Free             | -                | **Ilimitado**<br /><small>por tempo limitado</small>    |
...
O [ID do modelo](/docs/config/#models) na sua configuração do Noxijent
usa o formato `noxijent-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`noxijent-go/kimi-k3` na sua configuração.
...
| Union Alpha Free             | Não usado              | 0 dias            |
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Noxijent também.
