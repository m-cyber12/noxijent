---
title: Plugins
description: Escreva seus próprios plugins para estender o noxijent.
---

Plugins permitem que você estenda o noxijent conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do noxijent.
...
- `.noxijent/plugins/` - Plugins em nível de projeto
- `~/.config/noxijent/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/noxijent/node_modules/`.
...
1. Configuração global (`~/.config/noxijent/noxijent.json`)
2. Configuração do projeto (`noxijent.json`)
3. Diretório de plugins global (`~/.config/noxijent/plugins/`)
4. Diretório de plugins do projeto (`.noxijent/plugins/`)
...
O noxijent executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do noxijent para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o noxijent.
...
:::note
Se você estiver usando o aplicativo desktop noxijent, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o noxijent de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao noxijent:
...
O helper `tool` cria uma ferramenta personalizada que o noxijent pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o noxijent junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://noxijent.ai/docs/sdk) para detalhes.
