---
title: Provedores
description: Usando qualquer provedor LLM no noxijent.
---

import config from "../../../../config.mjs"
export const console = config.console

noxijent usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do noxijent.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/noxijent/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do noxijent.
...
Noxijent Zen é uma lista de modelos fornecidos pela equipe do noxijent que foram testados e verificados para funcionar bem com o noxijent. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Noxijent Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Noxijent Zen` e acesse [noxijent.ai/zen](https://noxijent.ai/zen).
...
Funciona como qualquer outro provedor no noxijent e é completamente opcional.

---

## Noxijent Go

Noxijent Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do noxijent que foram
testados e verificados para funcionar bem com o noxijent.

1. Execute o comando `/connect` no TUI, selecione `Noxijent Go` e acesse [noxijent.ai/zen](https://noxijent.ai/zen).
...
Para usar o Amazon Bedrock com o noxijent:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o noxijent:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `noxijent.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no noxijent não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o noxijent para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o noxijent funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do noxijent.
...
armazenar o token no armazenamento de autenticação do noxijent.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
noxijent usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o noxijent
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `noxijent.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `noxijent.json`:
...
Para usar sua assinatura do GitHub Copilot com o noxijent:
...
Para usar o Google Vertex AI com o noxijent:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o noxijent.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do noxijent, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [noxijent-helicone-session](https://github.com/H2Shami/noxijent-helicone-session) para registrar automaticamente cada conversa do noxijent como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do noxijent listada como uma sessão separada.
...
Você pode configurar o noxijent para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o noxijent para usar modelos locais através do LM Studio.
...
Você pode configurar o noxijent para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o noxijent. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/noxijent) para detalhes.
:::
...
Para usar o Ollama Cloud com o noxijent:
...
3. Copie a chave da API para uso no noxijent.
...
6. **Importante**: Antes de usar modelos em nuvem no noxijent, você deve puxar as informações do modelo localmente:
...
Noxijent Zen é uma lista de modelos testados e verificados fornecidos pela equipe do noxijent. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Noxijent Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Noxijent Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do noxijent.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do noxijent.
...
5. Você também pode personalizá-los através da sua configuração do noxijent. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o noxijent:
...
Você também pode personalizar modelos através da sua configuração do noxijent. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o noxijent. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no noxijent.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `noxijent.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o noxijent entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `noxijent auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do noxijent e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do noxijent.
