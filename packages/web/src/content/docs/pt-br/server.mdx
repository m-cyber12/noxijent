---
title: Servidor
description: Interaja com o servidor noxijent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `noxijent serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente noxijent pode usar.
...
```bash
noxijent serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `noxijent.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
noxijent serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `NOXIJENT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `noxijent`, ou defina `NOXIJENT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `noxijent serve` quanto ao `noxijent web`.

```bash
NOXIJENT_SERVER_PASSWORD=your-password noxijent serve
```
...
Quando você executa `noxijent`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor noxijent para interagir com o noxijent programaticamente.
:::

Essa arquitetura permite que o noxijent suporte múltiplos clientes e permite que você interaja com o noxijent programaticamente.

Você pode executar `noxijent serve` para iniciar um servidor autônomo. Se você tiver o TUI do noxijent em execução, `noxijent serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do noxijent [IDE](/docs/ide).
...
O servidor noxijent expõe as seguintes APIs.
