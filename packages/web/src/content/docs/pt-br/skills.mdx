...
As habilidades do agente permitem que o noxijent descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O noxijent pesquisa nesses locais:

- Configuração do projeto: `.noxijent/skills/<name>/SKILL.md`
- Configuração global: `~/.config/noxijent/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o noxijent sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.noxijent/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.noxijent/skills/git-release/SKILL.md` assim:
...
O noxijent lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `noxijent.json`:
...
**Para agentes embutidos** (em `noxijent.json`):
