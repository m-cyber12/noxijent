...
Com o noxijent, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o noxijent usa nosso próprio tema `noxijent`.
...
O noxijent vem com vários temas embutidos.
...
- Querem que o noxijent corresponda à aparência do seu terminal
...
O noxijent suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/noxijent/themes/*.json` ou `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.noxijent/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.noxijent/themes/*.json`
