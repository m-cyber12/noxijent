...
Создайте `.noxijent/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию noxijent или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем noxijent [config](/docs/config):
...
- Глобальный: `~/.config/noxijent/commands/`
- Для каждого проекта: `.noxijent/commands/`
...
noxijent включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
