---
title: Server
description: Interact with noxijent server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `noxijent serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an noxijent client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `noxijent.local` |
...
Set `NOXIJENT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `noxijent`, or set `NOXIJENT_SERVER_USERNAME` to override it. This applies to both `noxijent serve` and `noxijent web`.
...
When you run `noxijent` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the noxijent server to interact with noxijent programmatically.
:::

This architecture lets noxijent support multiple clients and allows you to interact with noxijent programmatically.

You can run `noxijent serve` to start a standalone server. If you have the
noxijent TUI running, `noxijent serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Noxijent [IDE](/docs/ide) plugins.
...
The noxijent server exposes the following APIs.
