...
With Noxijent you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Noxijent uses our own `noxijent` theme.
...
Noxijent comes with several built-in themes.
...
- Want Noxijent to match their terminal's appearance
...
Noxijent supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/noxijent/themes/*.json` or `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.noxijent/themes/*.json`
4. **Current working directory** - Defined in `./.noxijent/themes/*.json`
