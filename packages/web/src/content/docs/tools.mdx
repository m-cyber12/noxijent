...
Tools allow the LLM to perform actions in your codebase. Noxijent comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Noxijent.
...
:::note
This tool is only available when `NOXIJENT_EXPERIMENTAL_LSP_TOOL=true` (or `NOXIJENT_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Noxijent or Noxijent Go provider, or when either the `NOXIJENT_ENABLE_EXA` or `NOXIJENT_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Noxijent:

```bash
NOXIJENT_ENABLE_EXA=1 noxijent
# or
NOXIJENT_ENABLE_PARALLEL=1 noxijent
```
