...
description: Noxijent, LSP sunucularınızla bütünleşir.
---

Noxijent, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Noxijent, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`NOXIJENT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve noxijent bir dosyayı açtığında:
...
LSP sunucularını, noxijent yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
