---
title: Eklentiler
description: Noxijent'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Noxijent'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Noxijent'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.noxijent/plugins/` - Proje düzeyinde eklentiler
- `~/.config/noxijent/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/noxijent/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/noxijent/noxijent.json`)
2. Project config (`noxijent.json`)
3. Global eklenti dizini (`~/.config/noxijent/plugins/`)
4. Proje eklenti dizini (`.noxijent/plugins/`)
...
Noxijent bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Noxijent SDK istemcisi.
...
Noxijent'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Noxijent masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Noxijent'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Noxijent'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Noxijent'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Noxijent'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://noxijent.ai/docs/sdk) bakın.
