---
title: Sağlayıcılar
description: noxijent'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

noxijent, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı noxijent yapılandırmanızda yapılandırın.
...
`~/.local/share/noxijent/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları noxijent'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Noxijent Zen, noxijent ekibi tarafından sağlanan modellerin bir listesidir.
noxijent ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Noxijent Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, noxijent'u seçin ve [noxijent.ai/auth](https://noxijent.ai/auth)'ye gidin.
...
noxijent'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Noxijent Go

Noxijent Go, noxijent ile iyi çalıştığı test edilmiş ve doğrulanmış, noxijent ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Noxijent Go`'yu seçin ve [noxijent.ai/auth](https://noxijent.ai/zen) adresine gidin.
...
Amazon Bedrock'u noxijent ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   noxijent'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `noxijent.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi noxijent'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
noxijent'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   noxijent'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Noxijent yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
noxijent, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. noxijent'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`noxijent.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`noxijent.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi noxijent'la kullanmak için:
...
Google Vertex AI'yi noxijent ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları noxijent çalıştırılırken ayarlayın.
...
Helicone'un noxijent aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her noxijent görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [noxijent-helicone-session](https://github.com/H2Shami/noxijent-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her noxijent görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
noxijent'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla noxijent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla noxijent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini noxijent için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/noxijent)'a bakın.
:::
...
Ollama Cloud'u noxijent ile kullanmak için:
...
3. noxijent'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: noxijent'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Noxijent Zen, noxijent ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Noxijent Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Noxijent Zen**'i arayın.

   ```txt
   /connect
   ```

3. noxijent API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   noxijent yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca noxijent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Noxijent ile kullanmak için:
...
Modelleri noxijent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı noxijent ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `noxijent.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları noxijent'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `noxijent auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için noxijent yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, noxijent yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
