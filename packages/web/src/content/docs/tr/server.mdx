---
title: Sunucu
description: noxijent sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`noxijent serve` komutu, noxijent istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `noxijent.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `NOXIJENT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `noxijent` değeridir; değiştirmek isterseniz `NOXIJENT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `noxijent serve` hem de `noxijent web` için geçerlidir.
...
`noxijent` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
noxijent ile programatik etkileşim için noxijent sunucusunu kullanın.
:::

Bu mimari, noxijent'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`noxijent serve` ile bağımsız bir sunucu başlatabilirsiniz. noxijent TUI açıksa, `noxijent serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum noxijent [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
noxijent sunucusu aşağıdaki API'leri sunar.
