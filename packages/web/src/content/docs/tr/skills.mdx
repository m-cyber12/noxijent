...
Ajan becerileri, noxijent'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
noxijent su konumlari tarar:

- Proje konfigurasyonu: `.noxijent/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/noxijent/skills/<name>/SKILL.md`
...
Proje yerel yollarinda noxijent, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.noxijent/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/noxijent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.noxijent/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
noxijent, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`noxijent.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`noxijent.json` icinde):
