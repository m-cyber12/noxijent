...
noxijent ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak noxijent kendi `noxijent` temasını kullanır.
...
noxijent birden fazla yerleşik temayla gelir.
...
- noxijent'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
noxijent, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/noxijent/themes/*.json` veya `$XDG_CONFIG_HOME/noxijent/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.noxijent/themes/*.json`
4. **Geçerli çalışma dizini** - `./.noxijent/themes/*.json`
