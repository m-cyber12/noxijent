...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. noxijent yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
noxijent içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `NOXIJENT_EXPERIMENTAL_LSP_TOOL=true` (veya `NOXIJENT_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca noxijent provider kullanılırken veya `NOXIJENT_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

noxijent başlatırken etkinleştirmek için:
