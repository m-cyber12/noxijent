...
To debug issues with Noxijent, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `noxijent --log-level DEBUG`.

---

## Storage

noxijent stores session data and other application data on disk at:
...
To uninstall the Noxijent CLI and remove its related files, run:
...
To remove Noxijent Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Noxijent Desktop runs a local Noxijent server (the `noxijent-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Noxijent` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/noxijent/noxijent.jsonc` (or `~/.config/noxijent/noxijent.json`)
- **macOS/Linux** (older installs): `~/.local/share/noxijent/noxijent.jsonc`
...
Noxijent can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Noxijent can rebuild it.

1. Quit Noxijent Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/noxijent`
- **Linux**: delete `~/.cache/noxijent` (or run `rm -rf ~/.cache/noxijent`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\noxijent`

3. Restart Noxijent Desktop.

---

### Fix server connection issues

Noxijent Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `noxijent.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `NOXIJENT_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `NOXIJENT_PORT` (or pick a free port) and restart.
...
On Windows, Noxijent Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Noxijent's features.

---

### Notifications not showing

Noxijent Desktop only shows system notifications when:

- notifications are enabled for Noxijent in your OS settings, and
...
1. Quit Noxijent Desktop.
2. Find and delete these files (they live in the Noxijent Desktop app data directory):

- `noxijent.settings.dat` (desktop default server URL)
- `noxijent.global.dat` and `noxijent.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Noxijent:
...
### Noxijent won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `noxijent upgrade`
...
To figure out what models you have access to, run `noxijent models`
...
If you encounter API call errors, this may be due to outdated provider packages. noxijent dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart noxijent to reinstall the latest provider packages

This will force noxijent to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
noxijent will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
