---
title: TUI
description: Using the Noxijent terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Noxijent provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Noxijent starts the TUI for the current directory.
...
When using the Noxijent TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Noxijent. Allows you to select from available providers and add their API keys.
...
Exit Noxijent. _Aliases_: `/quit`, `/q`
...
This is separate from `noxijent.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Noxijent waits after the leader key. Defaults to `2000`.
...
Use `NOXIJENT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `noxijent.default`.
