...
const model = "noxijent/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with noxijent...`)
  ...
  try {
    await $`noxijent run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  noxijent failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with noxijent")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with noxijent...")
