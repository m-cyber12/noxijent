...
import { createNoxijent } from "@noxijent-ai/sdk"
...
`)
  ...
  const noxijent = await createNoxijent({ port: 0 })
  ...
  try {
    ...
    const session = await noxijent.client.session.create()
    const result = await noxijent.client.session
      .prompt({
