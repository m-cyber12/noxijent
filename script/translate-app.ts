...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "noxijent/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".noxijent/glossary/zh-cn.md"
  if (locale === "zht") return ".noxijent/glossary/zh-tw.md"
  return `.noxijent/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Noxijent did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Noxijent returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Noxijent session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Noxijent runs for 'all' (default: 4)
      --model <provider/id>  Noxijent model (default: noxijent/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Noxijent
...
`)
  ...
  if (failed.length) console.error(`\nNoxijent failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.NOXIJENT_DISABLE_PROJECT_CONFIG = "1"
  env.NOXIJENT_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["noxijent", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.NOXIJENT_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["noxijent", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.NOXIJENT_CONFIG
  delete env.NOXIJENT_CONFIG_DIR
  delete env.NOXIJENT_CONFIG_CONTENT
  delete env.NOXIJENT_PERMISSION
  delete env.NOXIJENT_AUTO_SHARE
  return env
}
