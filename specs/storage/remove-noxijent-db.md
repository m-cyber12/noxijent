# Remove `packages/noxijent/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/noxijent/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the noxijent legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/noxijent/src/storage/db.ts` are concentrated in 22 source files:

- `packages/noxijent/src/account/repo.ts`
- `packages/noxijent/src/cli/cmd/db.ts`
- `packages/noxijent/src/cli/cmd/import.ts`
- `packages/noxijent/src/cli/cmd/stats.ts`
- `packages/noxijent/src/control-plane/workspace.ts`
- `packages/noxijent/src/index.ts`
- `packages/noxijent/src/node.ts`
- `packages/noxijent/src/permission/index.ts`
- `packages/noxijent/src/project/project.ts`
- `packages/noxijent/src/server/projectors.ts`
- `packages/noxijent/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/noxijent/src/server/shared/fence.ts`
- `packages/noxijent/src/session/message-v2.ts`
- `packages/noxijent/src/session/projectors.ts`
- `packages/noxijent/src/session/prompt.ts`
- `packages/noxijent/src/session/session.ts`
- `packages/noxijent/src/session/todo.ts`
- `packages/noxijent/src/share/share-next.ts`
- `packages/noxijent/src/storage/db.ts`
- `packages/noxijent/src/sync/index.ts`
- `packages/noxijent/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy noxijent database wrapper; `packages/noxijent/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the noxijent wrapper.
...
Status: Completed. `SyncEvent` and the noxijent projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy noxijent database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy noxijent database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy noxijent database wrapper.
...
All migration groups are complete or superseded. `packages/noxijent/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No noxijent data-migration group remains.

The previous noxijent `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate noxijent data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/noxijent`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/noxijent/src`
- `bun typecheck` from `packages/noxijent`
- Relevant package tests from `packages/noxijent`, not the repo root
