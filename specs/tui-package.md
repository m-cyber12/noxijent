...
Move the canonical Noxijent terminal application from
`packages/noxijent/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/noxijent ---\
                      > @noxijent-ai/tui -> @noxijent-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/noxijent`,
`packages/cli`, or `@noxijent-ai/core`.

The SDK is the TUI's Noxijent boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/noxijent`.
- Do not replace private `packages/noxijent` imports with `@noxijent-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@noxijent-ai/tui`.

## Ownership Boundary

### `@noxijent-ai/tui` Owns
...
- Noxijent domain data displayed by the TUI
...
```text
packages/noxijent/src/cli/cmd/tui
```

Its private dependency on `packages/noxijent` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/noxijent/src/*`.
...
Status: Completed. The private `@noxijent-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@noxijent-ai/tui`.
...
- The package has no dependency on `noxijent`, `@noxijent-ai/cli`, or
  `@noxijent-ai/core`.
...
primitives now live in `@noxijent-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@noxijent-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Noxijent globals or process state;
...
Replace process-global Noxijent state with resolved TUI inputs.
...
`@noxijent-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/noxijent` initially.
...
canonical in `@noxijent-ai/tui`. Configured references resolve through the new
...
adapters rather than Noxijent domain implementations.

Make the SDK the only Noxijent domain boundary used by the TUI.
...
- Domain-facing TUI code imports Noxijent data and operations only from
  `@noxijent-ai/sdk`.
...
`@noxijent-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/noxijent`.
...
- The package has no imports from `packages/noxijent`, `packages/cli`, or
  `@noxijent-ai/core`.
...
public `@noxijent-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/noxijent`.
- Keep the legacy embedded worker and server startup in `packages/noxijent`.
- Change those adapters to load config, create transport inputs, and call the
  public `@noxijent-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/noxijent`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@noxijent-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@noxijent-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Noxijent domain state.
...
cd packages/noxijent && bun typecheck
...
rg '@noxijent-ai/core|packages/noxijent|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/noxijent/src packages/noxijent/test
```
...
- Build the current-platform `packages/noxijent` binary.
