...
Both `action` and `resource` use noxijent's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.noxijent` files is intentionally deferred until `.noxijent` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether noxijent permits an operation through its managed execution paths.
