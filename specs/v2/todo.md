...
The noxijent server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@noxijent-ai/llm` tool loop after replacing its
...
- plugins get global "noxijent" instance like in that post i showed
- noxijent instance has stuff like `noxijent.session.prompt()` or
  `noxijent.tool.register({...})`
...
- decide whether to preserve deprecated `@noxijent-ai/llm` orchestration exports
